        ...
        <aside :class="{ 'translate-x-0': sidebarOpen, '-translate-x-full': !sidebarOpen }"
            ...
            <nav class="p-4 space-y-1">
                ...
                <a href="{{ route('laporan.index') }}"
                    ...
                    <span class="font-medium text-sm">Laporan Buku Besar</span>
                </a>
                ...
                <a href="{{ route('laporan.neraca') }}"
                    class="flex items-center p-3 {{ request()->routeIs('laporan.neraca') ? 'bg-primary-50 text-primary-700 hover:text-primary-700' : 'text-gray-600 hover:bg-gray-50 hover:text-primary-700' }} rounded-lg group transition-colors">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('laporan.labarugi') }}"
                    class="flex items-center p-3 {{ request()->routeIs('laporan.labarugi') ? 'bg-primary-50 text-primary-700 hover:text-primary-700' : 'text-gray-600 hover:bg-gray-50 hover:text-primary-700' }} rounded-lg group transition-colors">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.addEventListener('submit', function(e) {
                ...
                // If button exists and not already processing
                if (submitBtn && !submitBtn.disabled && !submitBtn.dataset.noLoading) {
                    ...
                    // For standard forms, the browser will navigate away or reload, so we don't need to revert
                    // potentially, unless the validation fails client-side (but native validation usually prevents submit event)
                    // or if it's an AJAX form (handled separately usually)
                }
            });
        });
    </script>
